...
async function init() {
  ...
  let isPlaying = true;
  // Increase segmentCount by 2 for the virtual book interaction segments (Spread 1 and Spread 2)
  const segmentCount = sequences.length + 2; 
  ...
  function drawImageCover(img) {
    ...
    if (canvasRatio > imgRatio) {
      // Canvas is wider than image (e.g. ultrawide monitor)
  ...
  function render() {
    ...
    let videoSeqIndex = targetSeqIndex;
    if (videoSeqIndex >= sequences.length) {
      videoSeqIndex = sequences.length - 1;
    }
    ...
    // For seq5, the video (panning and book opening) should finish in the first 65% of the scroll space
    // For seq6 and seq7 (virtual segments), the video is paused on the last frame (videoProgress = 1.0)
    ...
    if (targetSeqIndex >= 5 && seq5Progress > 0.65) {
      const fadeProgress = Math.max(0, Math.min(1, (seq5Progress - 0.65) / 0.30));
      // Ease out cubic for a cinematic crossfade
    ...
    if (spreadData[0] && spreadData[1]) {
      ...
      if (targetSeqIndex < 6) {
        ...
        spreadData[1].el.style.opacity = 1;
